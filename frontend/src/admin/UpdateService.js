...
const UpdateService = ({ match }) => {
    ...
    const newPostForm = () => (
        <form className="mb-3 update_form" onSubmit={clickSubmit}>
            <h4 className="postphoto1">Post Photo</h4>
            ...
            <button className="btn btn-outline-warning btn-block uroom">Update Rooms</button>
        </form>
    );
    ...
    const showSuccess = () => (
        <div className="alert alert-info" style={{ display: createdService ? '' : 'none' }}>
            <h5>{`${createdService}`} is updated!</h5>
        </div>
    );
